...
const create = (baseURL = staging) => {
  ...
  //jalanyuks
  const getPlace = payload => api.get(`/place?key=${payload.key}&id=${payload.id}&lat=${payload.lat}&lng=${payload.long}&limit=10&page=${payload.page}`,'',{headers:{Authorization:`Bearer ${payload.token}`}})
